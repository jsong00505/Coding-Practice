...
public class Main {
  ...
  static boolean isSameColor(char firstColor, char secondColor, boolean colorBlind) {
    ...
    if(colorBlind) {
      if(firstColor != secondColor) {
        if((firstColor == RED && secondColor == GREEN) || (firstColor == GREEN && secondColor == RED)) {
          result = true;
        }
      } else {
        result = true;
  ...
  static boolean[][] depthFirstSearch(int n, int x, int y, char[][] map, boolean[][] visited, char color, boolean colorBlind) {
    visited[x][y] = true;
